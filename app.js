...
const DBN="tcesp-adolfo-v6",STORE="dados",KEY="base";
...
function tceExpenseUrl(year){return `https://transparencia.tce.sp.gov.br/municipio/adolfo/${year}/despesas`}
...
(async()=>{try{
  db=await idbGet();
  if(!db){
    let old=localStorage.getItem("tcesp-adolfo-data")||localStorage.getItem("tcesp-data");
    if(old){
      db=JSON.parse(old);
    }else{
  ...
  await save();
  localStorage.removeItem("tcesp-adolfo-data");
}catch(e){
